...
function hydraOjack() {
    ...
    speed = 0.05
    osc(4, 0.1, 0.8).color(1.04,0, -1.1).rotate(0.30, 0.1).pixelate(2, 20).modulate(noise(2.5), () => 1.5 * Math.sin(0.08 * time))
    .saturate(0.05)
    .out(o0)
}
...
function hydraLines() {
    ...
    src(o1).colorama(1.2).posterize(4).saturate(0.7).contrast(6).mult(solid(),0.15)
    .saturate(0.05)
    .out(o0)
}
...
function hydraLiquid() {
    ...
    speed = 0.05
    noise(3,0.3,3).thresh(0.3,0.03).diff(o3,0.3)
    .saturate(0.2)
    .out(o1)
    gradient([0.3,0.3,3]).diff(o0).blend(o1)
    .saturate(0.2)
    .out(o3)
    voronoi(33,3,30).rotate(3,0.3,0).modulateScale(o2,0.3).color(-3,3,0).brightness(3).out(o0)
    shape(30,0.3,1).invert(({time})=>Math.sin(time)*3)
    .saturate(0.2)
    .out(o2)
    render(o3)
...
function hydraLavaLamp() {
    ...
    speed = 0.01
    osc(1).add(noise(5, 2)).color(0, 0, 3).colorama(0.4)
    .saturate(0.05)
    .out()
}
